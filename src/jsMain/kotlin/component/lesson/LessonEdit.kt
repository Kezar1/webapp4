...
val CLessonEdit = FC<LessonEditProps>("LessonEdit") { props ->
    ...
    if (query.isLoading) div { +"Loading..." }
        ...
        val items = Json.decodeFromString<Array<Item<Teacher>>>(query.data ?: "")
        val teacher = items.findLast { it.id == props.item.elem.teacherId }
        if (teacher!=null)
            CTeacherList {
                this.teacher = teacher.elem.fullname()
            }
    }
}
